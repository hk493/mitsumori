import io, os, json, streamlit as st
from logic import load_config, extract_data_text, extract_data_ocr, write_to_excel_with_mapping

#  テンプレExcelを固定（OneDriveフルパススラッシュ表記ならエスケープ不要）
TEMPLATE_PATH = r"C:/Users/1109944/OneDrive - 光通信/見積excel.xlsx"

st.set_page_config(page_title="estimation-proto (固定テンプレ版)", layout="wide")
st.title("estimation-proto（固定テンプレ：見積excel.xlsx）")
...
with col_left:
    st.subheader("1) 入力")
    st.caption(f" Excelテンプレは固定: {TEMPLATE_PATH}")
    if not os.path.exists(TEMPLATE_PATH):
        st.error(f"テンプレファイルが見つかりません: {TEMPLATE_PATH} をこのPCに配置してください。")

    cfg_file = st.file_uploader("config.json（任意未指定ならローカルの config.json を使用）", type=["json"])
    if cfg_file:
        try:
...
with col_center:
    ...
    st.markdown("---")
    if st.button("固定テンプレに書き込み  Excel生成", type="primary", use_container_width=True):
        if not os.path.exists(TEMPLATE_PATH):
            st.error(f"テンプレがありません: {TEMPLATE_PATH} を配置してください。")
