...
# OCR関連（Tesseractは必要、Popplerは不要化）
try:
...
try:
    import pypdfium2 as pdfium  # ← これでPopplerなしでPDF→画像を生成
...
def _render_images_without_poppler(pdf_bytes: bytes, dpi: int) -> List["Image.Image"]:
    """PDFium(pypdfium2)でPDFを画像化（Poppler不要）"""
    if not PDFIUM_AVAILABLE:
        raise RuntimeError("pypdfium2 が未導入です。requirements.txt を確認してください。")
    images = []
    pdf = pdfium.PdfDocument(io.BytesIO(pdf_bytes))
    scale = dpi / 72.0  # 72dpi基準 → 任意dpi
...
def _extract_text_with_ocr(pdf_bytes: bytes, ocr_cfg: Dict[str, Any]) -> Tuple[str, List[Dict[str, Any]]]:
    if not OCR_AVAILABLE:
        raise RuntimeError("Tesseract（pytesseract）が未導入です。")
    ...
    poppler_path = ocr_cfg.get("poppler_path")  # あっても無くてもOKにする

    if tcmd:
        pytesseract.pytesseract.tesseract_cmd = tcmd

    # 1) pdf2image（Poppler前提）を試す → 失敗したら 2) PDFium で代替
    images = None
    if PDF2IMAGE_AVAILABLE:
        try:
            ...
            images = None
    if images is None:
        images = _render_images_without_poppler(pdf_bytes, dpi=dpi)
...
def _extract_pair(raw_text: str, words: List[Dict[str, Any]], targets: Dict[str, Any],
    ...
    out: Dict[str, Any] = {}
    out["法人名"] = _extract_corp(raw_text, words, targets.get("法人名", {}))
